...
import type { EnrollmentReportData } from '@/lib/api/reports';
...
export const EnrollmentReportTable: React.FC<EnrollmentReportTableProps> = ({
  data,
  isLoading = false,
  onExportCSV,
  onExportPDF,
      ...
      <Card>
        <CardContent className="py-12">
          <div className="text-center text-gray-500">
            No enrollment data found for the selected date range.
          </div>
        </CardContent>
      </Card>
    );
              ...
              {data.map((enrollment, index) => (
                <TableRow key={index} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-sm font-semibold text-right">
                    ₹ {parseFloat(enrollment.amount.toString()).toFixed(2)}
                  </TableCell>
                  <TableCell className="text-sm">
                    <span
                    ...
                    >
                      {enrollment.payment_mode === 'ONLINE'
                        ...
                        ? '💵'
                        : '⏳'}{' '}
                      {enrollment.payment_mode || 'NOT_PAID'}
                    </span>
                  </TableCell>
                  <TableCell className="text-sm">
                    <span
...
export default EnrollmentReportTable;
