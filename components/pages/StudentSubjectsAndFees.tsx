...
export default function StudentSubjectsAndFees({ setCurrentPage }: StudentSubjectsAndFeesProps) {
  ...
  if (error || !stats) {
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 p-4 rounded-lg flex items-center gap-2 text-red-700 font-inter">
          <AlertCircle size={20} />
          {error || 'No data found'}
        </div>
      </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 px-4 py-2 bg-emerald-50 rounded-2xl border border-emerald-100">
           ...
           <span className="text-[10px] font-bold text-emerald-700 uppercase tracking-widest font-inter">Records Synchronized</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div className="bg-white rounded-[28px] p-6 border border-slate-100 shadow-lg shadow-slate-200/10 ring-1 ring-blue-400/10 group relative overflow-hidden">
          ...
          <p className="text-2xl font-black text-slate-900 font-poppins">{stats.subjects_count}</p>
        ...
        <div className="bg-white rounded-[28px] p-6 border border-slate-100 shadow-lg shadow-slate-200/10 ring-1 ring-blue-400/10 group relative overflow-hidden">
          ...
          <p className="text-2xl font-black text-slate-900 font-poppins">{formatCurrency(stats.total_fee)}</p>
      ...
      <div className="space-y-6">
        {stats.enrolled_subjects && stats.enrolled_subjects.length > 0 ? (
          stats.enrolled_subjects.filter(Boolean).map((enr, idx) => {
            ...
            return (
              <div key={idx} className="bg-white rounded-[32px] p-6 sm:p-10 border border-slate-100 shadow-xl shadow-slate-200/20 ring-1 ring-blue-400/10 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/5 group">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                  {/* Left: Subject Info */}
                  <div className="lg:col-span-2 space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                        <div className="w-14 h-14 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100 shadow-sm group-hover:scale-110 transition-transform">
                            <BookOpen size={24} />
                        </div>
                        <div>
                            <h3 className="text-xl sm:text-3xl font-black text-slate-900 uppercase tracking-tight font-poppins">{subjectName}</h3>
                    ...
                    <div className="space-y-4">
                        ...
                        <p className="text-sm sm:text-base text-slate-600 font-medium leading-relaxed font-inter max-w-2xl">
                    ...
                    <div className="flex flex-wrap gap-4 pt-2">
                        <div className="px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                            ...
                            <p className="text-xs font-black text-slate-900 uppercase font-poppins">{enr.status}</p>
                        </div>
                        <div className="px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest font-inter">Schedule</p>
                            <p className="text-xs font-black text-slate-900 font-poppins uppercase">{enr.batch_time || '7-8 AM'}</p>
                        </div>
                    </div>
                  </div>

                  {/* Right: Fee Breakdown */}
                  <div className="bg-slate-50 rounded-[28px] p-6 sm:p-8 border border-slate-100 space-y-6 relative overflow-hidden">
                    ...
                    <div className="space-y-4 relative z-10">
                        <div className="flex justify-between items-center text-sm font-inter">
                            ...
                            <span className="font-black text-slate-900">{formatCurrency(registrationFee)}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm font-inter">
                            ...
                            <span className="font-black text-slate-900">₹{libraryFee.toLocaleString('en-IN')}</span>
                        </div>
                        <div className="pt-4 border-t border-slate-200 flex justify-between items-center">
                            ...
                            <span className="text-xl sm:text-2xl font-black text-slate-900 font-poppins">{formatCurrency(enr.total_fee)}</span>
                        </div>
                    </div>
          ...
          <div className="py-20 text-center bg-white rounded-[32px] border-2 border-dashed border-slate-100 font-inter">
