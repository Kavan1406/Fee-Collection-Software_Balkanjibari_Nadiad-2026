...
class SubjectBatchViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter batches by subject from URL parameter or query params with optimized enrollment counting."""
        from django.db.models import Count, Q
        
        queryset = SubjectBatch.objects.all().annotate(
            enrolled_count_annotated=Count(
                'subject__enrollments',
                filter=Q(
                    subject__enrollments__batch_time=models.F('batch_time'),
                    subject__enrollments__is_deleted=False,
                    subject__enrollments__status='ACTIVE'
                )
            )
        )
